## Script (Python) "getEventsBeforeAQ"
...
##title=Get Events that start before date using AdvancedQuery
##
""" 
...
modified for CalendarX 0.6.1(dev) change 'ALL' behavior in xsub, and filtering
  for restricted subject list, and 'end' as a parameter for speedups
...
#XCREATOR works
#  if 'xcrt' (Creator) == '1', then we want to show ONLY those events 
...
"""   
#  "end" parameter is for bigQB queries to pull all eventsBefore for a period (month, week, etc)
if end:
    dateBefore = end
else:
    dateBefore = start
#query = Le('start', start) 
query = Le('start', dateBefore) 
query = query & Ge('end', start) 
